...
class CurrencyWidget extends LitElement {
  static properties = {
    ...
    _from: {type: String, state: true },
    _to: {type: String, state: true},
    _amount: {type: Number, state: true},
    currencyList: {type: Array, state: true},
  }
  ...
  constructor() {
    ...
    this._to="USD";
    this._amount=1;
    this.countriesFlags = [{"Afghanistan": {"mini": "http://flags.fmcdn.net/data/flags/mini/af.png", "normal": "http://flags.fmcdn.net/data/flags/normal/af.png"}, 
...
"Zimbabwe": {"mini": "http://flags.fmcdn.net/data/flags/mini/zw.png", "normal": "http://flags.fmcdn.net/data/flags/normal/zw.png"}}];
  ...
  renderRate(){
    const url="https://api.exchangerate.host/convert?from="+this._from+"&to="+this._to;
    fetch(url)
      .then(response => response.json())
      .then(data => {
          this._data = data.info.rate; 
      });
  }
  fromCurrency(e){
  ...
  render() {
    if(this._data){
      ...
      <p>Enter an amount: <input id="input" type="text" value="1" @input="${this.enterAmount}"></p>
      
      <label>From:</label>
      
        <select id="currency-select" @change="${this.fromCurrency}">
        ${
          this.currencyList.map(currency => 
            currency ==="AUD" ?
            html `<option value="${currency}" selected> ${currency}
            </option>`
            :
            html
        ...
        <label>To:</label>
        <select id="currency-select" @change="${this.toCurrency}">
          ${
            this.currencyList.map(currency =>
            currency ==="USD" ?
            html `<option value="${currency}" selected> ${currency}
          ...
          <p class="result">
      ${this._from} is
       <strong> ${this._amount*this._data}</strong>
        ${this._to}
        </p>
      `
    }
...
customElements.define('widget-currency', CurrencyWidget);